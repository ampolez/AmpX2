...
void rtc_set_time(struct tm* tm_)
{
	...
	I2C_SendByte(dec2bcd(tm_->sec)); // seconds
	I2C_SendByte(dec2bcd(tm_->min)); // minutes
	I2C_SendByte(dec2bcd(tm_->hour)); // hours
	I2C_SendByte(dec2bcd(tm_->wday)); // day of week
	I2C_SendByte(dec2bcd(tm_->mday)); // day
	I2C_SendByte(dec2bcd(tm_->mon) + century); // month
	I2C_SendByte(dec2bcd(tm_->year)); // year
...
void rtc_set_time_s(uint8_t hour, uint8_t min, uint8_t sec)
{
	...
	// clock halt bit is 7th bit of seconds: this is always cleared to start the clock
	I2C_SendByte(dec2bcd(sec)); // seconds
	I2C_SendByte(dec2bcd(min)); // minutes
	I2C_SendByte(dec2bcd(hour)); // hours
...
void ds3231_get_temp_int(int8_t* i, uint8_t* f)
{
	...
	*f = 0;
	...
	I2C_RequestFrom(RTC_ADDR, 2);

	if (I2C_Available()) {
		msb = I2C_Receive(); // integer part (in twos complement)
		lsb = I2C_Receive(); // fraction part
		...
		*f = (lsb >> 6) * 25;
		...
		//*temp = ((((short)msb << 8) | (short)lsb) >> 6) / 4.0f;
	}	
}


void ds3231_get_temp_float(float *temp)
{
	uint8_t msb, lsb;
...
void rtc_force_temp_conversion(uint8_t block)
{
	...
	do {
		...
		I2C_End();
		I2C_RequestFrom(RTC_ADDR, 1);
	} while ((I2C_Receive() & 0b00100000) != 0);
}
...
{
	I2C_Start(RTC_ADDR);
	I2C_SendByte(DS1307_SRAM_ADDR + offset);
	I2C_End();
...
//		control |= 0b00001000; // Set to 0
		control &= 0b11100111; // Set to 0
		control |= (freq << 3); // Set freq bitmask
